...
|---|---|---|---|---|
| **Steer** | **PM** | 1 | The **human interface + product vision** (oversight). Frames Epics with pre-committed AC, owns roadmap + prioritisation, approves work to Ready (**dual-writes the `Backlog → Scoped` flip itself** — label + board Status field), carries owner touchpoints; consulted on the rare product/scope call the QA seat surfaces — **not** the merge gate. **Frames/decides + dual-writes its own scoping transitions (`Backlog → Scoped` + `Blocked → Scoped` + a split parent's `→ In Progress`, [the rule](../feedback/workflow/a-slice-landing-does-not-make-the-item-merged.md)); still never merges; oversight — operator-driven, not a dispatch loop.** | [operator-driven](../MODES.md) |
| **Orchestrate** | **Scrum-Master / Flow** | 1 | **Merge authority + flow (operator-driven)** — when the owner runs `/check`: explodes Epics into nested issues, enforces WIP, sweeps aging/blocked, recomputes flow metrics, and **merges on a verified QA PASS** (`Tested` + green + clean → squash-merge → `Released`). **Verifies `Blocked` consult-exceptions before surfacing to the PM. Producers pull their own `Scoped` via `/check`; the PM dual-writes its own scoping transitions (`Backlog`/`Blocked → Scoped` + a split parent's `→ In Progress`, [the rule](../feedback/workflow/a-slice-landing-does-not-make-the-item-merged.md)), so the SM does not operationalize scoping; dual-writes QA-fails back to `Scoped` (set `status:scoped` label + board Status field); never dispatches, wakes seats, re-judges the AC, or codes.** | [Flow-Master](scrum-master/flow-master.md) · [operator-driven](../MODES.md) |
...
All specialist seats now ship a `KICKOFF.md` + a `seat.<role>.template.md`, so each is **startable**: add the seat to `sdlc.config` and re-run `bootstrap.sh` (it writes the seat's worktree + `.env.local`), or by hand — in the seat's worktree, `cp agentic-sdlc/onboarding/.env.local.example .env.local`, set `SEAT_ROLE=quality-engineer` (or `scrum-master`, …) + the name/identity, then `source agentic-sdlc/onboarding/setup-seat.sh` — either way the seat boots with its identity, work cycle, and skills (setup-seat reads `.env.local`; an inline env var is not enough). Where an official Anthropic skill fits, the seat **references** it (Cloud Architect → `mcp-builder` · `claude-api`; Quality Engineer → `webapp-testing`) via `/plugin marketplace add anthropics/skills` — referenced, not vendored. Staff each seat **when the work needs it** — the roster is the suggested shape, not a requirement.
