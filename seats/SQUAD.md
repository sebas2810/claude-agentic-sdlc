...
|---|---|---|---|---|
| **Steer** | **PM** | 1 | The **human interface + product vision** (oversight). Frames Epics with pre-committed AC, owns roadmap + prioritisation, approves work to Ready, carries owner touchpoints; consulted on the rare product/scope call the QA seat surfaces — **not** the merge gate. **Oversight — operator-driven, not a dispatch loop.** | [operator-driven](../MODES.md) |
| **Orchestrate** | **Scrum-Master / Flow** | 1 | **Merge authority + flow (operator-driven)** — when the owner runs `/check`: explodes Epics into nested issues, enforces WIP, sweeps aging/blocked, recomputes flow metrics, and **merges on a verified QA PASS** (`Tested` + green + clean → squash-merge → `Released`). **Producers pull their own `Scoped` via `/check`; routes QA-fails to `Scoped`; never dispatches, wakes seats, re-judges the AC, or codes.** | [Flow-Master](scrum-master/flow-master.md) · [operator-driven](../MODES.md) |
