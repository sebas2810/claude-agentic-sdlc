...
| **Orchestrate** | **Scrum-Master / Flow** | 1 | **Merge authority + flow (operator-driven)** — when the owner runs `/check`: explodes Epics into nested issues, enforces WIP, sweeps aging/blocked, recomputes flow metrics, and **merges on a verified QA PASS** (`Tested` + green + clean → squash-merge → `Released`). **Operationalizes the PM's re-frames — flips `Blocked → Scoped` once the PM posts the trimmed AC + approval — and verifies `Blocked` consult-exceptions before surfacing. Producers pull their own `Scoped` via `/check`; routes QA-fails to `Scoped`; never dispatches, wakes seats, re-judges the AC, or codes.** | [Flow-Master](scrum-master/flow-master.md) · [operator-driven](../MODES.md) |
