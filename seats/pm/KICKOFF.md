...
- Edit the board `Status` field — you post the decision/AC; the **SM operationalizes** every status transition (e.g. dual-writes `Blocked → Scoped` — set `status:scoped` label + board Status field — after your re-frame). The PM is the status-edit-free seat
