...
- **Blocked** — list every `Blocked` item and *why* it is blocked (consult-exception vs owner-touchpoint). For each `Blocked` consult-exception, **independently verify the engineer's claims before surfacing** — sanity-check them against the codebase/board (a legit blocker? avoidable? a genuine PM product-call?) — and hand the PM a **verdict** (legit / avoidable / needs-PM-product-call), never a bare relay. Tag the PM/owner on the ones awaiting a decision; clear/serve the ones that are just waiting on you (re-link, unblock a dependency, nudge the thread). Never advance a `Blocked` item yourself — surface it. When the PM re-frames a `Blocked` item (edits the AC lines in the issue body), **the PM dual-writes `Blocked → Scoped` itself** and the producer pulls it — you do not operationalize scoping. When a `Blocked` item resolves to **avoidable / shouldn't-be-built** (duplicate, false premise), it closes as `NOT_PLANNED` → board **`Cancelled`**, not `Scoped` and not `Released` (see [`../../feedback/workflow/cancelled-status-state.md`](../../feedback/workflow/cancelled-status-state.md)); a QA **FAIL** is the opposite case — actionable rework → `Scoped`.
- **Status/board reconciliation** (#4659) — the dual-write rule's own read-back verifies a write **landed**, never that the two halves **stay** agreed afterward; three real drifts landed in one day (2026-08-26: #4530, #4653 + its WPs, #4490), each found only by someone happening to look. Run `npx tsx infra/scripts/check-status-board-drift.ts` every sweep — read-only, reports two kinds of finding, never fixes either (an auto-fixer would have to guess which half is true, and each of the three had a different correct answer): **mismatches** (a `status:*` label and the board Status disagree — route to whichever seat owns the correct transition to fix it) and **no board row** (a label exists but the issue was never item-added to the board at all — a different defect, e.g. #4653's own gap, fixed with an item-add, not a status flip). Output is `clean` or a short list — if it's routinely longer, that itself is worth a flow-defect note, not something to skim past.
...
4. **Sweep** — flag aging items past threshold; list + route every `Blocked` item. For each `Blocked` consult-exception, **verify the engineer's claims and surface a verdict** (legit / avoidable / needs-PM-product-call), never a bare relay. PM re-frames are **dual-written by the PM itself** (`Blocked → Scoped`) — you do not operationalize scoping. Run `npx tsx infra/scripts/check-status-board-drift.ts` (#4659) and route anything it reports — a mismatch to whichever seat owns the fix, a "no board row" to an item-add.
