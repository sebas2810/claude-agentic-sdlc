...
1. **On `/check`, pull your next item** — the next `Scoped` item in your lane off the board; claim it (flip `In Progress`; the assignee stays the human owner), then `git fetch origin && git switch -c infra/<epic#>-<slug> origin/main` (never local main — stale-base trap). `/board` is the operator's overview.
