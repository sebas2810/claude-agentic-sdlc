...
|---|---|---|
| [`workflow/finish-report-stop.md`](workflow/finish-report-stop.md) | Finish → one check → report → STOP and wait for the human (no loops, no polling); merge authority = 4-eye Engineer builds → QA verifies → SM merges (the SM merges on the QA PASS; engineer never self-merges; PM is oversight, not the merge gate — **the PM posts decisions/AC only and does not edit the board `Status`; the SM operationalizes status transitions, e.g. flips `Blocked → Scoped` after a PM re-frame**) | Light-confirm only on the no-loops part; merge authority moved to the SM in v1.1 (was Engineer→PM); PM = decisions-only / SM operationalizes added in v1.2 |
