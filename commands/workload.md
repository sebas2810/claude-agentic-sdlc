...
- **engineer** (KEY = `dex`/`sam`/…):
  - **`Scoped` · `seat:$KEY`** — your outstanding build queue (what `/check` would pull next). *This is the "items scoped on your name" list.*
  - **`In Progress` · `seat:$KEY`** — what you already have in flight.
- **quality-engineer**: all **`Delivered`** items — your verify queue.
- **pm**: all **`Tested`** items — your merge queue; plus a one-line count of **`Backlog`** awaiting framing.
- **scrum-master**: a flow view — count per state (`Scoped`/`In Progress`/`Delivered`/`Tested`/`Merged`) + any **aging** or **`Blocked`** items to surface.
