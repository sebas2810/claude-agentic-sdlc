...
**Dual-write, every transition.** A status change sets the `status:*` **label** (REST — the discovery mirror) **and** the board **Status field** (one cheap single-item mutation — get the item's project-item id with a targeted single-issue query, *never* the 300-item list — keeps the visual kanban + canonical record live). *(Optional: if the label→board sync Action is enabled — see [`../onboarding/board-label-sync.md`](../onboarding/board-label-sync.md) — the label write alone suffices and you skip the field mutation entirely.)*
