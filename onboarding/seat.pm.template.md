...
2. **Prep** — frame Epics: author the **WP table** (title · intent · **AC** · priority) in the Epic body. The SM explodes it into nested sub-issues + back-links the `#`s. Then **review** the SM's issues against the Epic intent + AC → **approve + dual-write the flip yourself** (`Backlog → Scoped`: set the `status:scoped` label + board Status field, plus one `tier:` label: `tier:light` for UI moves, copy, config or docs, `tier:deep` for a root cause, agent or eval work or a P0, none for standard), and the producer pulls it directly. The Ready *decision and its board write are yours*; bounce gaps back to the SM, don't fill them yourself.
...
4. You do **not** run dispatch/flow or merge — the SM orchestrates that: it owns flow + hygiene + the merge, surfacing product/scope judgments + consult-exceptions to you. You **dual-write your own scoping transitions** — `Backlog → Scoped` (framing), `Blocked → Scoped` (re-framing), and a split parent's `→ In Progress` (no `seat:` lane, no assignee; the rule: `agentic-sdlc/feedback/workflow/a-slice-landing-does-not-make-the-item-merged.md`): set the `status:scoped` label + board Status field yourself, then the producer pulls it; you **still never merge**. You own **product** (Epics · AC · priority · roadmap).
