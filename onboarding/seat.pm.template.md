...
2. **Prep** — frame Epics: author the **WP table** (title · intent · **AC** · priority) in the Epic body. The SM explodes it into nested sub-issues + back-links the `#`s. Then **review** the SM's issues against the Epic intent + AC → **approve + dual-write the flip yourself** (`Backlog → Scoped`: set the `status:scoped` label + board Status field), and the producer pulls it directly. The Ready *decision and its board write are yours*; bounce gaps back to the SM, don't fill them yourself.
