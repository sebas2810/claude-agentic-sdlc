...
SEAT_FILE="$ROOT/.orbis-seat.md"
TEMPLATE="$ROOT/agentic-sdlc/onboarding/orbis-seat.${SEAT_ROLE}.template.md"
if [ -f "$SEAT_FILE" ]; then
  echo "    seat:   .orbis-seat.md present (kept)"
elif [ -f "$TEMPLATE" ]; then
  sed -e "s/<NAME>/${SEAT_NAME}/g" -e "s/<ROLE>/${SEAT_ROLE}/g" "$TEMPLATE" > "$SEAT_FILE"
  echo "    seat:   .orbis-seat.md scaffolded (${SEAT_ROLE}) — set its steer line to your EPIC"
else
...
if command -v jq >/dev/null 2>&1; then
  ...
  mkdir -p "$ROOT/.claude"
  [ -f "$SETTINGS" ] || printf '{}\n' > "$SETTINGS"
  if grep -q 'orbis-seat\.md' "$SETTINGS" 2>/dev/null; then
    echo "    hook:   SessionStart already wired"
  else
    _hook_cmd='cat "$CLAUDE_PROJECT_DIR/.orbis-seat.md" 2>/dev/null || true'
    _tmp="$(mktemp)"
    jq --arg cmd "$_hook_cmd" \
      '.hooks.SessionStart = ((.hooks.SessionStart // []) + [{"hooks":[{"type":"command","command":$cmd}]}])' \
      "$SETTINGS" > "$_tmp" && mv "$_tmp" "$SETTINGS"
    echo "    hook:   SessionStart → injects .orbis-seat.md (wired)"
  fi
else
