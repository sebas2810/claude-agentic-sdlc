...
3. When the owner runs **`/check`** here: **drain your queue** — each discovery is one cheap `gh issue list --search` on the `status:*` label index (never the 300-item board read), re-run per item until no ready `Tested` merge and no flow task remains for the SM (cheap per-item ops throughout; each merge still 4-eye-gated — not autonomous draining):
...
5. Drain, then report + idle — keep re-running the cheap discovery query and clearing the next ready `Tested` merge and the next flow task until none remain for the SM, then report and idle; do **not** keep re-querying once your queue is clear (stop at empty, no idle-poll) — the owner runs `/check` again when new work lands. Sign all GitHub activity as <NAME>, never as the owner.
