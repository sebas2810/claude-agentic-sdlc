...
#   - Quality floor: a quality worker on an area:agentic item runs at standard
#     or above. A wrong PASS on agent work is the cheapest to make and the
...
# a second tier label, a tier: label it does not know, an unknown role or
# argument, a --fails that is not a whole number, or --fails for the quality
# role. Running a tier nobody chose, silently, is the weakened-default failure
...
for raw in "${items[@]+"${items[@]}"}"; do
  ...
  case "$label" in
    tier:light|tier:standard|tier:deep)
      ...
      tier="${label#tier:}"
      ;;
    tier:*) refuse "'$label' is not a tier (tier:light, tier:standard or tier:deep)" ;;
    area:agentic) agentic=1 ;;
  esac
done
