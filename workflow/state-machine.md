...
| Blocked → Scoped | **PM re-frames + dual-writes it itself** — the PM edits the AC lines in the issue body (a ruling only in a comment is not a re-frame) and sets the `status:scoped` label + board `Status` field; the producer then re-pulls it | the re-framed AC lines in the issue body + dual-written (the PM scopes its own items) |
