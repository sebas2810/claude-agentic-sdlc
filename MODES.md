...
2. **`/check`** in a seat pane → that seat pulls + does its **next workload**. The operator runs **`/clear` first**, so every drain starts from a fresh context (the board and the threads carry the state). A seat does its own work in its own context; between items it can `/clear` again when the context has grown large:
   - **producer** (`engineer`) → next `Scoped` for its `seat:` lane → claim → build → `Delivered`. **Block protocol:** on a genuine consult-exception (AC unmeetable as written · a real product fork · out-of-scope creep) it does **not** build — it posts the **full context to the GitHub issue** (file-cited findings · options · recommendation), sets `Blocked`, and stops; the issue comment is the board item's context.
   ...
   - **pm** → oversight + product: frame the next `Backlog` → `Scoped` with its pre-committed AC, re-frame a `Blocked` consult-exception the SM surfaced, own the roadmap + owner touchpoints, resolve the rare product/scope judgment the QA seat surfaces (not in the routine merge path). The PM **dual-writes its own scoping transitions** — `Backlog → Scoped` (framing), `Blocked → Scoped` (re-framing), and a split parent's `→ In Progress` (no `seat:` lane; [the rule](feedback/workflow/a-slice-landing-does-not-make-the-item-merged.md)): set the `status:*` label **and** the board `Status` field together; producers then pull `status:scoped` directly. The PM **still never merges** — the SM is the merge authority.
...
- **Role boundary — the PM scopes its own work; the SM is the merge authority.** The **PM dual-writes its
  own scoping transitions** — for `Backlog → Scoped` (framing), `Blocked → Scoped` (re-framing), and a split parent's `→ In Progress` (no `seat:` lane; [the rule](feedback/workflow/a-slice-landing-does-not-make-the-item-merged.md)) it sets
