...
2. **`/check`** in a seat pane → that seat pulls + does its **next workload**. PM and scrum-master panes: the operator runs **`/clear` first**, so every drain starts from a fresh context (the board and the threads carry the state). Producers and the quality seat get a fresh context per item from their [workers](workflow/fresh-context-workers.md):
